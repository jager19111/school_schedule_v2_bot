...
#-------РЕНДЕРЫ РЕГИСТРАЦИИ-------------
    ...
    def render_family_created(
        ...
        return UIRenderer.render_registration_success(
            ...
            next_step=(
                ...
                "чтобы позвать ребёнка, второго родителя или родственника-наблюдателя."
            ),
        )

    #==========================================================================
    # Рендеры по jinja2 шаблонам
    #==========================================================================

    # рендер справки
    @staticmethod
    def render_help_page(dto: HelpPageDTO) -> str:
        """
        Короткая справка для Telegram.
        Вся логика текстов и секций инкапсулирована в Jinja2-шаблоне.
        """
        template = template_env.get_template('help_page.j2')
        return template.render(dto=dto).strip()
   
    
    # мониторинг Nika кеш для админа
    @staticmethod
    def render_nika_source_health(
        ...
        Вся логика форматирования и статусов вынесена в Jinja2-шаблон.
        """
        template = template_env.get_template('nika_source_health.j2')
        # Прокидываем метод UIRenderer.format_dt прямо в контекст шаблона
        return template.render(dto=dto, format_dt=UIRenderer.format_dt).strip()
    ...
    def render_profile_reset_confirmation(
        ...
        Вся логика текстов инкапсулирована в Jinja2-шаблоне.
        """
        template = template_env.get_template('profile_reset_confirmation.j2')
        return template.render(dto=dto, successor_name=successor_name).strip()
...
# Сводка
    ...
    def render_day_changes_detail(
        ...
        Логика построения дерева подгрупп и сравнения полей 
        инкапсулирована в Jinja2-шаблоне.
        """
        template = template_env.get_template('day_changes_detail.j2')
        text = template.render(dto=dto)
...
#==============================================================   
    ...
    def render_system_error_plain() -> str:
        ...
        return (
            ...
            "Попробуйте повторить действие ещё раз. "
            "Если повторяется - напишите в поддержку"
        )
