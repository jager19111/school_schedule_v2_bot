...
class UIRenderer:
   ...
   # старый код для примера. удалить
    @staticmethod
    def render_family_code_prompt() -> str:
        """
        Общий prompt для присоединения по family_code.

        Код выдаёт администратор семьи. Он подходит для child,
        parent и observer, но не заменяет персональную claim-ссылку
        ребёнка, для которого уже создан virtual student profile.
        """
        return (
            "👨‍👩‍👧 <b>Подключение к семье</b>\n\n"
            "Введите код семьи, который вам сообщил "
            "администратор семьи.\n\n"
            "Если у вас нет кода, попросите администратора создать "
            "и отправить вам персональное приглашение.\n\n"
            "ℹ️ Если для ребёнка уже создан профиль в семье без Telegram, "
            "лучше использовать персональную ссылку для привязки Telegram. "
            "Она объединит профили и сохранит все дополнительные занятия."
        )
        ...
        if intended_role == "child":
            if has_standalone_child_profile:
                lines.extend([
                    ...
                    "✅ Расписание и дополнительные занятия сохранятся.",
...
# Уведомления
 ...
 # перерегистрация профиля   
    ...
    def render_profile_reset_confirmation(
        ...
        if dto.role == "child":
            # --- ИЗМЕНЕНИЕ ЗДЕСЬ ---
            # Добавлена обработка ребенка, который состоит в семье
...
# Виртуальный ученик
    ...
    def render_student_claim_confirmation(
        *,
        student_name: str,
        class_name: str,        # ИЗМЕНЕНО: вместо class_id
        group_name: str,        # ИЗМЕНЕНО: вместо group_id
        ...
        # ИЗМЕНЕНО: Если ALL — "Весь класс". Иначе выводим само название, 
        # так как мы теперь передаем человекочитаемое имя из NIKA.
        group_text = (
            "Весь класс"
            if group_name == "ALL"
